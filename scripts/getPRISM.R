...
drugs <- read.csv(file.path(download_dir, sec_screen_drug), stringsAsFactors = FALSE)
...
# lab durgnames
lab.drug.names <- read.csv(file.path(download_dir, "drugs_with_ids.csv"), stringsAsFactors = F, na.strings = "") # this file is "drug_with_ids.csv" from pachy-annotations
...
############################################# Cell object #############################################
# Lab cell names
lab.cell.names <- read.csv(file.path(download_dir, "cell_annotation_all.csv"), stringsAsFactors = F, na.strings = "") # this file is "cell_annotation_all.csv" from pachy annotation
...
cells <- read.csv(file.path(download_dir, sec_screen_cell), stringsAsFactors = FALSE)
...
dose.resp.data <- read.csv(file.path(download_dir, sec_screen_dr), stringsAsFactors = F)
...
raw.sensitivity <- readRDS(file.path(download_dir, "raw.sensitivity.prismii_v3.rds"))
...
profile.sensitivity <- readRDS(file.path(download_dir, "profile.sensitivity.PRISM.rds"))
...
auc.data <- read.csv(file.path(download_dir, sec_screen_auc), stringsAsFactors = F)
...
######### Cell #########
cur.cell <- data.frame(
  unique.cellid = cell.obj$cellid,
  PRISM.cellid = cell.obj$PRISM.cellid,
  row.names = cell.obj$cellid
)

######### Tissue #########
cur.tissue <- data.frame(
  unique.tissueid = cell.obj$tissueid,
